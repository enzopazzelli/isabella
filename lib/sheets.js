...
function parseMarcas(rows) {
  if (!rows) return null
  return rows
    .map(r => ({
      id: String(r.ID || ''),
      nombre: String(r.Nombre || ''),
      logo: getDriveImageUrl(String(r.Logo || '')) || '',
      orden: Number(r.Orden) || 0,
    }))
    .sort((a, b) => a.orden - b.orden)
}

function parsePromos(rows) {
  if (!rows) return null
  return rows
    .sort((a, b) => (Number(a.Orden) || 0) - (Number(b.Orden) || 0))
    .map(r => String(r.Texto || ''))
    .filter(Boolean)
}

export async function fetchAllData() {
  const [productos, hero, categorias, banners, testimonios, instagram, faq, config, marcas, promos] =
    await Promise.all([
