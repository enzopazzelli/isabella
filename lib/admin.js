...
export function getSheetUrl(sheetName) {
  if (!SHEET_ID) return '#'
  const range = sheetName ? `?range=${encodeURIComponent(sheetName)}!A1` : ''
  return `https://docs.google.com/spreadsheets/d/${SHEET_ID}/edit${range}`
}

export function getSheetEditUrl(sheetName) {
  return getSheetUrl(sheetName)
}
